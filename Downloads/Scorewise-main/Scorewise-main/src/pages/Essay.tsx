...
import { AssignmentDiffView } from "@/components/institution/AssignmentDiffView";
...
const Essay = () => {
  ...
  const assignmentData = location.state as {
    ...
    assignmentOriginalTopic?: string | null;
    assignmentOriginalInstructions?: string | null;
  ...
  const submitAssignmentEssay = useCallback(async () => {
    ...
    try {
      ...
      // Ensure essay is saved locally (already done above with updateEssay)
      // Now ensure it's saved to cloud
      ...
      } else if (!essayId && user) {
        // Even if offline, try to save - it will queue for later
        try {
          essayId = await uploadEssay(updatedEssay);
          if (essayId) {
            updateEssay(currentLocalId, { cloudId: essayId });
          ...
          console.log('Cloud save will happen when online');
        }
      }

      // For assignments, we need cloudId, but for regular essays, continue even without cloudId
      if (assignmentData?.isAssignment && !essayId) {
        console.error('Could not save essay to cloud - required for assignment submission');
        toast({
          title: 'Save error',
          description: 'Could not save to cloud. Please check your connection.',
          variant: 'destructive'
        });
        return;
      }
  ...
  const handleTimeEnd = useCallback(async () => {
    ...
    // Ensure essay is saved locally first
    if (currentLocalId && essay.trim()) {
      const currentEssayData = getEssay(currentLocalId);
      if (currentEssayData) {
        const updatedEssay = {
          ...currentEssayData,
    ...
    // Sync to cloud if logged in (for all essays, not just assignments)
    if (user && currentLocalId) {
      try {
        const currentEssayData = getEssay(currentLocalId);
        if (currentEssayData && !currentEssayData.cloudId) {
          // Upload to cloud if not already uploaded
          const cloudId = await uploadEssay(currentEssayData);
          if (cloudId) {
      ...
      await syncEssays(essays, saveEssays);
        ...
        console.log('Cloud sync will happen when online');
      }
    }

    // Auto-submit if this is an assignment
    await submitAssignmentEssay();
  }, [forceSave, user, isOnline, essays, saveEssays, syncEssays, submitAssignmentEssay, currentLocalId, essay, getEssay, updateEssay, uploadEssay]);
  ...
  }, [forceSave, user, isOnline, essays, saveEssays, syncEssays, currentLocalId, essay, getEssay, updateEssay, uploadEssay]);
        ...
        {assignmentData?.isAssignment && (
          <Card className="mb-6 border-primary/20 bg-primary/5">
            ...
            <CardContent className="space-y-3">
              <AssignmentDiffView
                currentText={assignmentData.assignmentTopic || ''}
                originalText={assignmentData.assignmentOriginalTopic}
                label="Topic"
              />
              {assignmentData.assignmentInstructions && (
                <AssignmentDiffView
                  currentText={assignmentData.assignmentInstructions}
                  originalText={assignmentData.assignmentOriginalInstructions}
                  label="Instructions"
                />
              )}
