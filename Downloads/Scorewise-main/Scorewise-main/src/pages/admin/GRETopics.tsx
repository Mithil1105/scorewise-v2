...
export default function GRETopics() {
  const { user } = useAuth();
  const [topics, setTopics] = useState<GRETopic[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [editingTopic, setEditingTopic] = useState<GRETopic | null>(null);
  ...
  const handleAdd = async () => {
    ...
    try {
      const { error } = await supabase.from('gre_topics').insert({ topic: newTopic.trim() });
      ...
      await logAdminAction(user!.id, 'CREATE_GRE_TOPIC', undefined, { topic: newTopic } as Record<string, string>);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await logAdminAction(user!.id, 'UPDATE_GRE_TOPIC', editingTopic.id, {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from('gre_topics').delete().eq('id', id);

      if (error) throw error;

      await logAdminAction(user!.id, 'DELETE_GRE_TOPIC', id);
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      await logAdminAction(user!.id, 'BULK_UPLOAD_GRE_TOPICS', undefined, {
        count: lines.length,
  ...
  return (
    <AdminLayout title="Manage GRE Topics">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>GRE Essay Topics ({topics.length})</CardTitle>
          <div className="flex gap-2">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <Textarea
                    placeholder="Enter the GRE topic..."
                    value={newTopic}
